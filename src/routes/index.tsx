...
import {
  Activity, Ambulance, ArrowRight, BedDouble, CircleAlert, Clock3, HeartPulse,
...
function Label({ children }: { children: React.ReactNode }) { return <p className="text-[11px] font-bold uppercase tracking-[.22em] text-muted-foreground">{children}</p>; }
...
function TriageOutput({data,onReset}:{data:TriageResult & {id:string;name:string;age:number;gender:string;vitals:Vitals};onReset:()=>void}) { const tone=data.category==="RED"?"critical":data.category==="YELLOW"?"warning":"success"; const room=data.category==="RED"?"R-03":data.category==="YELLOW"?"Y-01":"G-03"; return <section className="mx-auto max-w-5xl space-y-5">
  ...
  <div className={`flex items-center justify-between rounded-lg border border-${tone}/40 bg-${tone}-soft p-6`}><div className="flex items-center gap-5"><strong className={`font-display text-6xl text-${tone}`}>{data.stage}</strong><div><h1 className="text-2xl font-bold">Stage {data.stage} — {data.category}</h1><p className="text-muted-foreground">{data.name} · age {data.age} · {data.gender}</p></div></div><div className="text-right"><span className="text-xs">Acuity score</span><strong className="block text-3xl">{data.score}</strong></div></div>
...
function Analytics({blocking,onGridlock}:{blocking:number;onGridlock:()=>void}) { return <><PageTitle eyebrow="Operational intelligence" title="Utilization & readiness" text="Monitor throughput, capacity, staffing, equipment, and emerging shortages."/><div className="mt-6 grid gap-5 lg:grid-cols-3"><Chart title="Bed utilization" values={[38,42,41,48,53,58,62,68]} /><Chart title="Clinical load" values={[46,49,55,51,64,67,73,78]} /><Chart title="Arrival volume" values={[20,35,28,46,53,49,72,66]} /></div><div className="mt-6 grid gap-5 lg:grid-cols-[1fr_360px]"><section className="rounded-lg border border-border bg-surface p-6"><h2 className="text-xl font-bold">Live operational updates</h2><div className="mt-5 space-y-4">{["RED capacity is at 33%; two ICU rooms remain ready.","Oxygen inventory is projected to reach reorder point in 19 hours.","Afternoon intake is tracking 12% above the seven-day average.","Ambulance KA-01-MD-4821 is inbound with one high-acuity patient."].map((x,i)=><div key={x} className="flex gap-4 border-b border-border pb-4"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-secondary text-xs">{i+1}</span><p className="text-sm">{x}</p></div>)}</div></section><section className="rounded-lg border border-warning/30 bg-warning-soft p-6"><CircleAlert className="text-warning"/><h2 className="mt-4 text-xl font-bold">Gridlock watch</h2><p className="mt-2 text-4xl font-bold">{(blocking*100).toFixed(1)}%</p><p className="mt-2 text-sm text-muted-foreground">Erlang-B: B(14.8 offered load, 16 staffed beds)</p><Button className="mt-5" onClick={onGridlock}>Review escalation</Button></section></div></>; }
